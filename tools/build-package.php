...
if ($name !== 'qr-code-generator' || preg_match('/\A\d+\.\d+\.\d+(?:[-+][0-9A-Za-z.-]+)?\z/', $version) !== 1) {
    throw new RuntimeException('Invalid plugin identity or version.');
}

$output = (string)($argv[1] ?? (sys_get_temp_dir() . '/' . $name . '-' . $version . '.zip'));
...
$packageFiles = [
    ...
    'assets/css/admin.css', 'assets/css/quick.css', 'assets/js/admin.js', 'assets/js/quick.js', 'assets/vendor/qrcode.js',
